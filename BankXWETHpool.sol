...
contract BankXWETHpool is IBankXWETHpool{
    ...
    function initialize(address _token0, address _token1, address _xsd_contract_address, address _pid_address) public override {
        ...
        XSDaddress = _xsd_contract_address;
        WETHaddress = _token1;
    ...
    function provideLiquidity() external override lock{
        ...
        uint balance0 = IERC20(bankxaddress).balanceOf(address(this));
    ...
    function provideLiquidity2(address to) external override lock{
        require(pid_controller.bucket2(), "There is no deficit detected");
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        uint balance0 = IERC20(bankxaddress).balanceOf(address(this));
        uint balance1 = IERC20(WETHaddress).balanceOf(address(this));
        uint amount1 = balance1.sub(_reserve1);
        uint ethvalue = amount1.mul(XSD.eth_usd_price());
        uint difference = pid_controller.diff2();
        liquidity_provider[to].ethvalue = ethvalue;
        liquidity_provider[to].bankxrewards = liquidity_provider[to].bankxrewards.add(ethvalue.mul(8)).div(100);
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external override lock {
        ...
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'BankXWETH: INSUFFICIENT_LIQUIDITY');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1), 'BankXWETH: K');
        }
    ...
    function flush() external override lock{
        //activate on every trade.
        // total_supply = minted - burned; amount currently in circulation
        //minted = total_supply + burned;
        //AND = total_supply - 2B;
        // get info from CD.
        (uint _reserve0, uint _reserve1,) = getReserves();
        uint XSDvalue = XSD.totalSupply().mul(XSD.xsd_price());
        require(_reserve0.add(_reserve1)<XSDvalue.div(5), "TVL not reached");
        require(BankX.totalSupply() > 2000000000e18, "Already deflationary");
        uint bankxamount = BankX.totalSupply().sub(2000000000e18);
        BankX.pool_burn_from(address(this), bankxamount);
    }
